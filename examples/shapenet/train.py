...
def main(_run, _config):
    ...
    filename_category = os.path.join(rootdir, "categories.txt")
    ...
    data_train, labels_shape_train, data_num_train, labels_pts_train, _ = data_utils.load_seg(
        filelist_train)
    data_val, labels_shape_val, data_num_val, labels_pts_val, _ = data_utils.load_seg(
        filelist_val)
    data_test, labels_shape_test, data_num_test, labels_pts_test, _ = data_utils.load_seg(
        filelist_test)
    data_train = np.concatenate([data_train, data_val], axis=0)
    labels_shape_train = np.concatenate(
        [labels_shape_train, labels_shape_val], axis=0)
    data_num_train = np.concatenate([data_num_train, data_num_val], axis=0)
    labels_pts_train = np.concatenate(
        [labels_pts_train, labels_pts_val], axis=0)
    ...
    def network_function(): return get_network(_config['model'], in_channels=1, out_channels=N_CLASSES,
    ...
    ds = Dataset(data_train, data_num_train, labels_pts_train, labels_shape_train,
                 npoints=_config['npoints'], training=True, network_function=network_function)
    train_loader = torch.utils.data.DataLoader(
        ds, batch_size=_config['batchsize'], shuffle=True, num_workers=_config['threads'])
    ds_test = Dataset(data_test, data_num_test, labels_pts_test, labels_shape_test,
                      npoints=_config['npoints'], training=False, network_function=network_function)
    test_loader = torch.utils.data.DataLoader(
        ds_test, batch_size=_config['batchsize'], shuffle=False, num_workers=_config['threads'])
    print("Done")

    print("Creating optimizer...", end='', flush=True)
    optimizer = torch.optim.Adam(
        net.parameters(), lr=_config['lr_start'], eps=1e-3)
    epoch_start = 0
    scheduler = torch.optim.lr_scheduler.MultiStepLR(
        optimizer, _config['milestones'], gamma=_config['gamma'], last_epoch=epoch_start-1)
    print("Done")
    ...
    for epoch in range(epoch_start, _config['epoch_nbr']):
        ...
        t = tqdm(train_loader, ncols=120, desc="Epoch {}".format(
            epoch), disable=_config['disable_tqdm'])
        for data in t:
            ...
            optimizer.zero_grad()
            outputs = net(features, pts, support_points=net_pts,
                          indices=net_ids)
        ...
        with torch.no_grad():
            cm = np.zeros((N_CLASSES, N_CLASSES))
            t = tqdm(test_loader, ncols=120, desc="Test {}".format(
                epoch), disable=_config['disable_tqdm'])
            for data in t:
                pts = data['pts'].to(device)
                features = data['features'].to(device)
                ...
                outputs = net(
                    features, pts, support_points=net_pts, indices=net_ids)
                loss = 0
                ...
                for i in range(pts.size(0)):
                    ...
                    loss = loss + weights[object_label] * \
                        F.cross_entropy(outputs_, seg_)
        ...
        logs.write(
            f"{epoch} {oa} {aa} {iou} {oa_test} {aa_test} {iou_test} \n")
        logs.close()
