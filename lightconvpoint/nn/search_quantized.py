...
class SearchQuantized:
    ...
    def __init__(self, K, stride=1, npoints=-1):
        self.K = K
        # use stride or npoints
        self.stride = stride
        self.npoints = npoints

    def __call__(self, points, support_points):

        if support_points is None and self.stride == 1 and self.npoints == -1:
        ...
        if support_points is None:
            ...
            if self.stride > 1 or self.stride == 1 and self.npoints == -1:
                support_point_number = max(
                    1, int(points.shape[2])//self.stride)
            else:
            ...
            support_points = batched_index_select(points.transpose(
                1, 2), dim=1, index=support_points_ids).transpose(1, 2)
