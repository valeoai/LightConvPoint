...
class LCP(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, bias=True, dim=3):
        ...
        # convolution layer
        self.cv = nn.Conv2d(in_channels, out_channels,
                            (1, kernel_size), bias=bias)
        ...
        self.fc2 = nn.Conv2d(2*self.kernel_size,
                             self.kernel_size, 1, bias=False)
        self.fc3 = nn.Conv2d(2*self.kernel_size,
                             self.kernel_size, 1, bias=False)
    ...
    def forward(self, input, points, support_points):
        ...
        if self.training:
            ...
            self.norm_radius.data = self.norm_radius.data * \
                (1-self.norm_radius_momentum) + \
        ...
        distance_weight = (distance_weight / distance_weight_s *
                           distances.shape[2]).unsqueeze(1)

        # feature weighting matrix estimation
        mat = F.relu(self.bn1(self.fc1(pts)))
        mp1 = torch.max(mat * distance_weight, dim=3,
                        keepdim=True)[0].expand((-1, -1, -1, mat.shape[3]))
        mat = torch.cat([mat, mp1], dim=1)
        mat = F.relu(self.bn2(self.fc2(mat)))
        mp2 = torch.max(mat * distance_weight, dim=3,
                        keepdim=True)[0].expand((-1, -1, -1, mat.shape[3]))
        mat = torch.cat([mat, mp2], dim=1)
        mat = F.relu(self.fc3(mat)) * distance_weight

        # compute features
        features = torch.matmul(input.transpose(
            1, 2), mat.permute(0, 2, 3, 1)).transpose(1, 2)
