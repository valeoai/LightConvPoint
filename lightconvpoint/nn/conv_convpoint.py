...
class ConvPoint(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, bias=True, dim=3):
        ...
        self.weight = nn.Parameter(
            torch.Tensor(in_channels * self.kernel_size, out_channels), requires_grad=True)
        ...
        for i in range(self.kernel_size):
            ...
            while (coord**2).sum() > 1:
                coord = np.random.rand(self.dim)*2 - 1
            center_data[:, i] = coord
        self.centers = nn.Parameter(torch.from_numpy(
            center_data).float(), requires_grad=True)
    ...
    def forward(self, input, points, support_points):
        ...
        pts = (pts.permute(0, 2, 3, 1).unsqueeze(
            4) - self.centers).contiguous()
        ...
        features = torch.matmul(features.view(
            features.size(0), features.size(1), -1), self.weight)
