...
class ResnetBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, K, ConvNet, Search, stride=1, npoints=-1):
        super(ResnetBlock, self).__init__()

        self.cv0 = nn.Conv1d(in_channels, in_channels//2, 1)
        self.cv1 = lcp_nn.Conv(ConvNet(in_channels//2, in_channels//2, kernel_size),
                               Search(K=K, stride=stride, npoints=npoints),
                               activation=nn.ReLU(), normalization=nn.BatchNorm1d(in_channels//2))
        self.cv2 = nn.Conv1d(in_channels//2, out_channels, 1)

        self.bn0 = nn.BatchNorm1d(in_channels//2)
        self.bn2 = nn.BatchNorm1d(out_channels)

        self.short = nn.Conv1d(
            in_channels, out_channels, 1) if out_channels != in_channels else nn.Identity()
        self.bn_short = nn.BatchNorm1d(
            out_channels) if out_channels != in_channels else nn.Identity()
        self.short_pool = lcp_nn.MaxPool() if (
            stride > 1) or (npoints > 0) else lcp_nn.Identity()
    ...
    def forward(self, x, input_pts, support_points=None, indices=None):
        ...
        if x is not None:
            ...
            shortcut, _, _ = self.short_pool(self.bn_short(
                self.short(shortcut)), input_pts, pts, indices=ids)
...
class KPConvCls(nn.Module):
    def __init__(self, in_channels, out_channels, ConvNet, Search):
        ...
        self.cv0 = lcp_nn.Conv(
            ConvNet(in_channels, pl, kernel_size), Search(K=K),
            activation=nn.ReLU(), normalization=nn.BatchNorm1d(pl))
        ...
        self.resnetb11 = ResnetBlock(
            2*pl, 2*pl, kernel_size, K, ConvNet, Search)
        self.resnetb20 = ResnetBlock(
            2*pl, 4*pl, kernel_size, K, ConvNet, Search, npoints=128)
        self.resnetb21 = ResnetBlock(
            4*pl, 4*pl, kernel_size, K, ConvNet, Search)
        self.resnetb30 = ResnetBlock(
            4*pl, 8*pl, kernel_size, K, ConvNet, Search, npoints=32)
        self.resnetb31 = ResnetBlock(
            8*pl, 8*pl, kernel_size, K, ConvNet, Search)
        self.resnetb40 = ResnetBlock(
            8*pl, 16*pl, kernel_size, K, ConvNet, Search, npoints=8)
        self.resnetb41 = ResnetBlock(
            16*pl, 16*pl, kernel_size, K, ConvNet, Search)
    ...
    def forward(self, x, input_pts, support_points=None, indices=None, return_features=False):
        ...
        x0, _, ids01 = self.resnetb01(
            x0, input_pts, input_pts, indices=indices[1])
        ...
        if x4 is not None:
            ...
            return None, [ids00, ids01, ids10, ids11, ids20, ids21, ids30, ids31, ids40, ids41], [pts1, pts2, pts3, pts4]


class KPConvSeg(nn.Module):  # Segsmall with config option for precomputing in the dataloader
    def __init__(self, in_channels, out_channels, ConvNet, Search):
        super(KPConvSeg, self).__init__()
    ...
    def forward(self, x, input_pts, support_points=None, indices=None, return_features=False):
        ...
        x0d, _, ids0u = self.upsample(
            x1d, pts1, input_pts, indices=indices[13])
        if x0d is not None:
            ...
            return None, [ids00, ids01, ids10, ids11, ids20, ids21, ids30, ids31, ids40, ids41,
                          ids3u, ids2u, ids1u, ids0u], [pts1, pts2, pts3, pts4]
